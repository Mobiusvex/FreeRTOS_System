...
void USART1_IRQHandler(void) {
    ...
    if (__HAL_UART_GET_FLAG(&huart1, UART_FLAG_IDLE)) // 再次确认是否为真正的 IDLE 中断标志
    {
        __HAL_UART_CLEAR_IDLEFLAG(&huart1); // 必须手动清除 IDLE 中断标志，否则会持续触发中断
        HAL_UARTEx_RxIDLECallback(&huart1);
    }
